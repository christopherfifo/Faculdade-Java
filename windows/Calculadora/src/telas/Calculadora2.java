...
public class Calculadora2  extends JFrame{
	...
	public void invert(String identificador) {
			if (display.getText() != "") {
				display.setText((Double.parseDouble(display.getText()) * -1) + "");
			}
	}
	
	public void memori( String identificador ) {
		if (identificador == "M1") {
			if (M1 == 0) {
			this.M1 = Double.parseDouble(display.getText());
			clear();
		} else {
			...
			if (M2==0) {
				this.M2 = Double.parseDouble(display.getText());
				clear();
			} else {
				display.setText(M2.toString());
				this.M2 = 0.0;
			}
		}
	...
	public String digito( String identificador ) {
		switch (identificador) {
			...
			case "AC": {clear(); } break;
			case "M1": {memori(identificador);} break;
			case "M2": {memori(identificador);} break;
			case "+/-": { invert(identificador); } break;
			default: {
				if(apagaDisplay) {
					display.setText("");
					apagaDisplay = false;
